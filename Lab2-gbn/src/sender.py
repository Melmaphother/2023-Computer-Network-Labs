...
class GBNSender(Device, OutMixIn):
    def __init__(
        ...
        self.seqno_start = 0
        # packet buffer to save the packets that haven't been acknowledged by receiver
    ...
    def run(self, env: Environment):
        ...
        """
        while self.seqno < self.seqno_start + self.window_size:
    ...
    def put(self, packet: Packet):
        """从接收端收到ACK"""
        ackno = packet.packet_id  # start from 0
        # print('1', ackno, self.seqno_start, self.seqno, self.absno)
        """
        TODO: 
        （1）检查收到的ACK
        ...
        """
        if (ackno + self.seqno_range - self.seqno_start) % self.seqno_range < self.window_size and (
                self.seqno + self.seqno_range - ackno) % self.seqno_range > 0:
            # print('before', len(self.outbound))
            for _ in range(0, 1 + (ackno + self.seqno_range - self.seqno_start) % self.seqno_range):
                self.outbound.popleft()
            # print('after', len(self.outbound))
        self.seqno_start = (ackno + 1) % self.seqno_range
        while ((self.seqno + self.seqno_range - self.seqno_start) % self.seqno_range < self.window_size and
               self.absno < len(self.message)):
            # print('2', ackno, self.seqno_start, self.seqno, self.absno)
        ...
        # print('3', ackno, self.seqno_start, self.seqno, self.absno, '\n')
